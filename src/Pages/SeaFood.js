...
import bannerseaFood2 from "../Assets/images/n_6.png";
...
import Data from "../Data/seafoodata";
...
const SeaFood = () => {
  ...
  const laravel = "https://api2.alvar.is/api/get-all-news";

  // ========================States=============================

  const [seafood, setSeafood] = useState(Data);
  ...
  useEffect(() => {
    ...
    axios.get(baseurl).then((response) => {
      setData1(response.data.data[1].attributes);
    });
  ...
  return (
    <>
      <Mistnavb
        ...
        link2={"/mist-focus/sea-food"}
        link3={"/mist-canal/sea-food"}
        link4={"/mist-direct/sea-food"}
        envlink={"/environmental-impact/sea-food"}
        faqlink={"/faqs/sea-food"}
        clientlink={"/clients/sea-food"}
      />
      <NavbarLine lineColor={"#0098DA"} display={"block"} />
      <Banner
        Src={bannerseaFood2}
        data1={seafood.banner}
        videoSrc={seafood.videoSrc}
      />
      ...
      <IndustryComp
        ...
        color={"#0098DA"}
      />
      <Protection data2={seafood.protection} />
      <Payment />
      <SysComp data={seafood.syscomps} />
      <BusinessProt data={seafood.Bussinesprot} />

      <ResourcePreserve
        data={seafood.Resourcepreservecard}
        data_para={seafood.Resourcepreserve}
      />
      <SimpleSwiper data={seafood.Simpleswiper} />
      {/* <AlvarCombat /> */}
      <TailerMade data={seafood.Tailermade} />
      <div className="mt-5" style={{ backgroundColor: "rgba(0,0,0,0.1)" }}>
        {lara && <CardsComp data={lara} />}
      </div>
      {/* <NewsLetter data={seafood.newsletter} /> */}
      <HowMist data={seafood.howmist} />
