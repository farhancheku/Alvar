...
import bannerseaFood2 from "../Assets/images/n__bm.png";
...
import Data from "../Data/Poultrydata";
...
const Poultry = () => {
  ...
  const [poultry, setPoultry] = useState(Data);
  ...
  useEffect(() => {
    ...
    axios.get(baseurl).then((response) => {
      ...
      setData1(response.data.data[4].attributes);
    });
  ...
  return (
    <>
      <Mistnavb
        bgColor={"rgba(236,181,51,0.45)"}
        comment0={"Mist Core"}
        comment1={"Mist Focus"}
        comment2={"Mist Canal"}
        comment3={"Mist Direct"}
        comment4={""}
        link1={"/mist-core/poultry"}
        link2={"/mist-focus/poultry"}
        link3={"/mist-canal/poultry"}
        link4={"/mist-direct/poultry"}
        envlink={"/environmental-impact/poultry"}
        faqlink={"/faqs/poultry"}
        clientlink={"/clients/poultry"}
      />
      <NavbarLine lineColor={"#ECB533"} display={"block"} />
      <Banner
       Src={bannerseaFood2} 
       data1={poultry.banner}
       videoSrc={poultry.videoSrc} />
      ...
      <IndustryComp
        ...
        color={"#ECB533"}
      />
      <Protection data2={poultry.protection} />
      <Payment />
      <SysComp data={poultry.syscomps} />

      <BusinessProt data={poultry.Bussinesprot} />

      <ResourcePreserve
        data={poultry.Resourcepreservecard}
        data_para={poultry.Resourcepreserve}
      />
      <SimpleSwiper data={poultry.Simpleswiper} />
      <TailerMade data={poultry.Tailermade} />

      <div className="mt-5" style={{ backgroundColor: "rgba(0,0,0,0.1)" }}>
        {lara && <CardsComp data={lara} />}
      </div>
      {/* <NewsLetter data={poultry.newsletter} /> */}
      <HowMist data={poultry.howmist} />
